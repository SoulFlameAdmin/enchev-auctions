...
export default function LiveAuctionsPage(){
  ...
  // syncClock intentionally uses refs so the interval keeps one stable lifecycle.
  ...
  const bid = async () => {
    ...
    try {
      ...
      // D28 owns explicit connection/reconnect/stale-state UI.
    }
  };
  ...
  return <main className="livePage" data-design-task="D24">
    ...
    <section className="liveHero">
      <div><span className="liveEyebrow">● LIVE AUCTION ROOM</span><h1>Наддавай в реално време</h1><p>10-секундният demo брояч се води от ENCHEV server session clock. При потвърдена оферта сървърът издава нов краен момент; при изтичане клиентът се ресинхронизира към активния лот.</p></div>
      <div className="liveHeroClock" data-design-countdown-task="D25" data-clock-mode={clockMode} role="timer" aria-label={`Остават ${remaining} секунди за лот ${current.lot}`}><small>{clockMode==="server"?"SERVER SYNC":"СИНХРОНИЗИРАНЕ"}</small><b>{fmt(remaining)}</b><span>LOT {current.lot}</span></div>
    </section>
    ...
    <section className="liveRoom" aria-labelledby="live-room-heading">
      ...
      <div className="liveRail">
        <aside className="liveBidPanel" aria-label="Панел за текущия лот">
          ...
          <div className="liveRule"><b>Как работи</b><p>Офертата се потвърждава през server clock endpoint и връща таймера на 10 сек. След 00:00 интерфейсът взема актуалния lot state от сървъра.</p></div>
        </aside>

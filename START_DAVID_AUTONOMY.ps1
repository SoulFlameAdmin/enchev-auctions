...
$env:DAVID_ACTIVE_WORKERS = "SYSTEM,APP2,APK"
$env:DAVID_CONTROL_ENABLED = "0"
$env:DAVID_CHATGPT_TAB_TARGET = "3"
...
for($i=0;$i-lt 180;$i++){
 ...
 if($sup-eq 1-and$sys-eq 1-and$dpp-eq 1-and$apk-eq 1-and$ctrl-eq 0-and$guard-eq 1-and$design-eq 0-and(CDP $Port)){
  $m=Join-Path $D ".david-tab-monitor.json";if(Test-Path $m){try{$t=Get-Content -Raw $m|ConvertFrom-Json;$cc=@($t.managed.CONTROL).Count;$sc=@($t.managed.SYSTEM).Count;$ac=@($t.managed.APP2).Count;$kc=@($t.managed.APK).Count;$dc=if($t.managed.PSObject.Properties.Name-contains"DESIGN"){@($t.managed.DESIGN).Count}else{0};if($cc-eq 0-and$sc-eq 1-and$ac-eq 1-and$kc-eq 1-and$dc-eq 0-and[int]$t.totalChatGptTabs-eq 3){$ok=$true;break}}catch{}}
 }
}
if(-not$ok){throw "AUTONOMY health gate failed: $last; expected CONTROL=0 DESIGN=0 WATCH=background and ChatGPT=3"}
...
Write-Host "[DAVID AUTONOMY] HEALTHY // ENCHEV SYSTEM + DPP + DAVID APK // WATCH BACKGROUND // ChatGPT=3" -ForegroundColor Green

...
while($true){
 ...
 Write-Host "============================================================" -ForegroundColor DarkGreen
 foreach($n in @("CONTROL","SYSTEM","APP2","APK")){$s=S $n;$w=if($s){[string]$s.watchdog}else{"NO STATE"};$a=if($s){[string]$s.lastAction}else{"-"};Write-Host ("{0,-8} {1,-30} {2}"-f $n,$w,$a) -ForegroundColor Cyan}
 if($t -and $t.managed){
  ...
  $ok=($cc-eq 1 -and $sc-eq 1 -and $ac-eq 1 -and $kc-eq 1 -and $dc-eq 0 -and [int]$t.totalChatGptTabs-eq 4)
  Write-Host ("TABS CTRL={0} SYS={1} DPP={2} APK={3} DESIGN={4} TOTAL={5} => {6}"-f $cc,$sc,$ac,$kc,$dc,$t.totalChatGptTabs,$(if($ok){"STABLE"}else{"CHECK"})) -ForegroundColor $(if($ok){"Green"}else{"Red"})
 }
 ...
 Write-Host ("PROC SUP={0} SYS={1} DPP={2} APK={3} CTRL={4} GUARD={5} DESIGN={6}"-f $p.SUP,$p.SYS,$p.DPP,$p.APK,$p.CTRL,$p.GUARD,$p.DESIGN) -ForegroundColor Green

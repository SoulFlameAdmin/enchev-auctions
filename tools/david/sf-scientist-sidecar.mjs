...
const MATRIX_LOG=path.join(HERE,".david-matrix-start.log");
const MAX_SHELLS=18;
...
function cleanText(value,max=700){
  let t=String(value??"").replace(/\u0000/g,"").replace(/\r/g,"").trim();
  t=t
...
function fileMtime(p){try{return fs.statSync(p).mtimeMs||0;}catch{return 0;}}
...
function listRuntimeFiles(){
  let names=[];
  try{names=fs.readdirSync(HERE);}catch{}
  const states=names.filter(n=>/^\.david-.*\.json$/i.test(n)&&!/^\.david-(?:scientist)/i.test(n)).slice(0,80);
  const logs=names.filter(n=>/\.log$/i.test(n));
  if(fs.existsSync(MATRIX_LOG)&&!logs.includes(path.basename(MATRIX_LOG)))logs.push(path.basename(MATRIX_LOG));
  return {states,logs};
}
function stateSummary(){
  const {states}=listRuntimeFiles();
  const rows=[];
  for(const name of states){
    const p=path.join(HERE,name),j=readJson(p,null);
    ...
    const row={
      ...
      inflightKey:cleanText(j.inflightKey||"",180)||null,
      chatUrl:typeof j.chatUrl==="string"?cleanText(j.chatUrl,180):null
    };
...
function logSummary(){
  const {logs}=listRuntimeFiles();
  const files=logs
    .map(name=>({name,path:path.join(HERE,name),mtime:fileMtime(path.join(HERE,name))}))
    .filter(x=>x.mtime>0)
...
async function shellProcesses(){
  const ps=[
    "$p=@(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue|Where-Object{$_.Name -match '^(powershell|pwsh|cmd|node)\\\\.exe
...
async function snapshot(){
  ...
  return {
    at:now(),
    mode:infer(p,tm),
    cdp9444Online:Boolean(ver),
    process:p,
    monitor:tm?{
      totalChatGptTabs:Number(tm.totalChatGptTabs||0),
      SYSTEM:Array.isArray(m.SYSTEM)?m.SYSTEM.length:0,
      DESIGN:Array.isArray(m.DESIGN)?m.DESIGN.length:0,
      APP2:Array.isArray(m.APP2)?m.APP2.length:0,
      APK:Array.isArray(m.APK)?m.APK.length:0,
      CONTROL:Array.isArray(m.CONTROL)?m.CONTROL.length:0,
      FREE_A:Array.isArray(m.FREE_A)?m.FREE_A.length:0,
      FREE_B:Array.isArray(m.FREE_B)?m.FREE_B.length:0
    }:null,
...
function event(prev,next){
  ...
  const prevT=telemetrySignature(prev.liveTelemetry),nextT=telemetrySignature(next.liveTelemetry);
  if(prevT!==nextT){
    const prevAlerts=digest(JSON.stringify(prev.liveTelemetry?.logAlerts||[]));
    const nextAlerts=digest(JSON.stringify(next.liveTelemetry?.logAlerts||[]));
    if(prevAlerts!==nextAlerts)return {important:true,reason:"DAVID live log/error telemetry changed"};
...
function boot(s){return "SF CORPORATION / AI SCIENTIST BOOTSTRAP\n\nYou are the independent AI Scientist observing the existing DAVID system. You are OUTSIDE DAVID. Preserve the existing DAVID architecture. Never claim an external action happened unless the local bridge reports it. Never bypass login/MFA/CAPTCHA/permissions. Observe, form hypotheses, test claims against evidence, detect regressions and propose improvements. The snapshot now includes LIVE TELEMETRY: active PowerShell/CMD/Node processes, DAVID runtime state summaries, recent log tails and error alerts. Use those details to explain what actually happened, not only topology.\n\nFor autonomous observations answer compactly in Bulgarian:\nВИДЯХ: ...\nРЕШИХ: ...\nЗАЩО: ...\nПРЕДЛАГАМ: ...\nRISK: LOW|MEDIUM|HIGH\n\nCurrent DAVID snapshot:\n"+JSON.stringify(s,null,2);}
function observe(reason,s){return "SF SCIENTIST AUTONOMOUS OBSERVATION\nEVENT: "+reason+"\n\nAnalyze only this evidence. Inspect liveTelemetry.shells, davidStates, recentLogs and logAlerts before concluding. Distinguish a process start/stop from a real script failure. If a log shows a concrete exception/error, name the process/file and evidence. You may decide no intervention is needed. Do not invent actions.\n\nDAVID SNAPSHOT:\n"+JSON.stringify(s,null,2)+"\n\nYou may autonomously choose ONE low-risk Scientist-sidecar action only when useful:\nACTION: NONE | OPEN_POWERSHELL | OPEN_CMD | OPEN_CHATGPT | SEARCH_WEB <query> | OPEN_URL <https-url> | DAVID_HEALTH_CHECK | GIT_STATUS | CONSULT_AB <question>\nThese actions affect only Scientist tools or read-only diagnostics; never modify DAVID architecture.\n\nReturn:\nВИДЯХ: ...\nРЕШИХ: ...\nЗАЩО: ...\nПРЕДЛАГАМ: ...\nRISK: LOW|MEDIUM|HIGH\nACTION: ...";}
function user(text,s){return "MITKO -> SF AI SCIENTIST\n"+text+"\n\nLive DAVID snapshot including PowerShell/CMD/Node processes, runtime state and recent logs:\n"+JSON.stringify(s,null,2)+"\n\nAnswer as SF AI Scientist. Separate observed facts from hypotheses. When relevant, cite the exact process PID/script, state file or log line that supports the conclusion. You can use one low-risk Scientist tool when useful by ending with ACTION: NONE | OPEN_POWERSHELL | OPEN_CMD | OPEN_CHATGPT | SEARCH_WEB <query> | OPEN_URL <https-url> | DAVID_HEALTH_CHECK | GIT_STATUS | CONSULT_AB <question>. Never modify DAVID architecture from this sidecar.";}
...
main().catch(e=>{save("Scientist fatal",{status:"offline",lastError:String(e?.stack||e)});process.exit(1);});
  ...
  try{
    const x=await execFileAsync("powershell.exe",["-NoProfile","-ExecutionPolicy","Bypass","-Command",ps],{windowsHide:true,timeout:6500,maxBuffer:1024*1024});
    const raw=String(x.stdout||"").trim();
    if(!raw)return [];
    const parsed=JSON.parse(raw),arr=Array.isArray(parsed)?parsed:[parsed];
    return arr
      .map(v=>({pid:Number(v.pid||0),ppid:Number(v.ppid||0),name:String(v.name||""),cmd:cleanText(v.cmd||"",520)}))
      .filter(v=>v.pid)
      .slice(0,MAX_SHELLS);
  }catch{return [];}
}
async function liveTelemetry(){
  const [shells,states,logs]=await Promise.all([
    shellProcesses(),
    Promise.resolve(stateSummary()),
    Promise.resolve(logSummary())
  ]);
  return {
    shells,
    davidStates:states,
...
function telemetrySignature(t){
  return digest(JSON.stringify({
    shells:(t?.shells||[]).map(x=>[x.pid,x.ppid,x.name,x.cmd]),
    states:(t?.davidStates||[]).map(x=>[x.file,x.status,x.watchdog,x.lastAction,x.lastError]),
    alerts:t?.logAlerts||[]
  }));
}

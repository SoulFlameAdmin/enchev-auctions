...
for(const token of ["DAVID MODE CENTER V2.1","SOULFLAME SYSTEM","DAVID A + B","STOP ALL","PINNED SAME CHATS","REFRESH STATUS","START_DAVID_AUTONOMY.ps1","RESTART_DAVID_AUTONOMY_CLEAN.ps1","START_DAVID_FREETALK_ONLY.ps1","RESTART_DAVID_FREETALK_ONLY_CLEAN.ps1","STOP_DAVID_ALL_CLEAN.ps1","Start-Mode","READY: CHOOSE SYSTEM OR DAVID A + B","Application]::DoEvents","PipelineStoppedException","-WindowStyle Hidden"]) if(!selectorV2.includes(token)) throw new Error("Mode selector V2 invariant missing: "+token);
...
console.log("DAVID_FREE_TALK_EXPERIMENT PASS normal_tabs=4 experiment_tabs=6 ab_only_tabs=2 persistent_chats=2 matrix_master=ON restart_first=ON update_before_choice=ON direct_start_when_stopped=ON selector_v2_1=STABLE no_timer=ON hidden_shells=ON live_status=ON stop_all=ON relay_dedupe=seq instant=FORCED rate_limit_auto_ack=ON rate_limit_auto_resume=ON same_tab_rollover=ON");

...
for(const token of ['DAVID_CHATGPT_TAB_TARGET="1"','DAVID_PROJECT_EFFORT_MODE="instant"','DAVID_GLOBAL_SEND_INTERVAL_MS="1500"']) if(!singleStart.includes(token)) throw new Error("FAST SOLO invariant missing: "+token);
if(!singleRestart.includes("STOP_DAVID_ALL_CLEAN.ps1")||!singleRestart.includes("START_DAVID_SINGLE.ps1")) throw new Error("FAST SOLO restart is not atomic");
console.log("DAVID_FREE_TALK_EXPERIMENT PASS normal_tabs=3 watch_background=ON experiment_tabs=6 ab_only_tabs=2 persistent_chats=2 powershell_only_entry=ON matrix_visible=GUARANTEED center_own_process=ON center_singleton=HARD old_david_shell_cleanup=ON selector_console_hidden=ON selector_ascii=ON matrix_master=ON restart_first=ON update_before_choice=ON always_clean_restart_on_choice=ON fresh_edge=VERIFIED fast_solo_buttons=3 fast_solo_tabs=1 fast_solo_instant=ON selector_v2_1=STABLE no_timer=ON hidden_shells=ON live_status=ON stop_all=ON relay_dedupe=seq instant=FORCED rate_limit_auto_ack=ON rate_limit_auto_resume=ON same_tab_rollover=ON");

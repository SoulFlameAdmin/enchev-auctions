...
for(const token of ["STOP FIRST","fetch origin","checkout $Branch","pull --ff-only","OPEN MATRIX VISIBLE","MATRIX READY","DAVID_MODE_SELECTOR_V2.ps1","Wait-OrchestrationMutex","& $Selector -Port $Port"]) if(!matrixMaster.includes(token)) throw new Error("Matrix master invariant missing: "+token);
if(matrixMaster.includes('Start-Process -FilePath $Pwsh -ArgumentList @("-NoProfile","-ExecutionPolicy","Bypass","-File",$Selector')) throw new Error("Matrix selector must run visibly in the current PowerShell host, never as a hidden child process");
...
console.log("DAVID_FREE_TALK_EXPERIMENT PASS normal_tabs=4 experiment_tabs=6 ab_only_tabs=2 persistent_chats=2 powershell_only_entry=ON matrix_visible=GUARANTEED matrix_master=ON restart_first=ON update_before_choice=ON always_clean_restart_on_choice=ON fresh_edge=VERIFIED selector_v2_1=STABLE no_timer=ON hidden_shells=ON live_status=ON stop_all=ON relay_dedupe=seq instant=FORCED rate_limit_auto_ack=ON rate_limit_auto_resume=ON same_tab_rollover=ON");

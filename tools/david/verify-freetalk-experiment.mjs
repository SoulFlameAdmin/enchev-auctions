...
console.log("DAVID_FREE_TALK_EXPERIMENT PASS normal_tabs=3 watch_background=ON experiment_tabs=6 ab_only_tabs=2 persistent_chats=2 powershell_only_entry=ON matrix_visible=GUARANTEED center_own_process=ON center_singleton=HARD old_david_shell_cleanup=ON selector_console_hidden=ON selector_ascii=ON matrix_master=ON restart_first=ON update_before_choice=ON always_clean_restart_on_choice=ON fresh_edge=VERIFIED selector_v2_1=STABLE no_timer=ON hidden_shells=ON live_status=ON stop_all=ON relay_dedupe=seq instant=FORCED rate_limit_auto_ack=ON rate_limit_auto_resume=ON same_tab_rollover=ON");

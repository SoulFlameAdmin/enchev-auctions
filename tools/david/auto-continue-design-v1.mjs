...
function isExternalBlocker(problem) {
  return /(redis|valkey|upstash|vercel|marketplace|environment-secret|environment secret|provider credential|credential|permission|authorization|quota|billing|plan limit|external access|legal sign-off|customer data|deployment.*queued|deployment.*initializing)/i.test(String(problem || ""));
}
...
async function runPrompt(context, page, state, prompt, kind) {
  for (;;) {
    ...
    if (/ChatGPT platform: (?:global )?rate limit/i.test(String(state.problem || ""))) {
      ...
      state.watchdog = "design-send-permit";
...
async function main() {
  ...
  while (!context) {
    try {
      ...
      console.log(`[DESIGN] CDP not ready: ${error?.message || error}. WAIT 5s -> reconnect. Worker stays alive.`);

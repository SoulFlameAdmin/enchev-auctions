...
Write-Host "[DAVID ALL] SESSION RESILIENCE V1: tagged managed chats survive fresh URLs; central guard classifies retryable errors, interruption, send-timeout, rate-limit, auth/human gates and unavailable chats." -ForegroundColor Yellow
...
Write-Host "[DAVID ALL] Recovery laws: active thinking/tool work=>WAIT; Retry UI=>bounded click; confirmed interruption=>refresh/verify/resend; unavailable/full chat=>same-tab fresh chat; CAPTCHA/MFA/login=>human gate." -ForegroundColor Yellow

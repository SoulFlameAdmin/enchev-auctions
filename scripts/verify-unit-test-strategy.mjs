...
export function validate(strategy, runnerSource, packageJson) {
  ...
  if (!packageJson?.scripts || packageJson.scripts.test !== "node scripts/verify-production-smoke-suite.mjs --self-test && node scripts/verify-failure-injection-harness.mjs --self-test && node scripts/run-ci-tests.mjs") fail("package test script must preserve the 25.13/25.14 pre-gates and canonical aggregate runner");

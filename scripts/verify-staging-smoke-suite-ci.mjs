...
export function validate(config, runtimeConfig, healthConfig, packageJson, preGateSource) {
  ...
  if (packageJson?.scripts?.test !== "node scripts/run-system-test-pre-gates.mjs && node scripts/run-ci-tests.mjs") fail("aggregate npm test runner drift");
  if (typeof preGateSource !== "string" || !preGateSource.includes('["scripts/verify-staging-smoke-suite-ci.mjs", "--self-test"]')) fail("25.12 must be registered in system pre-gates");
...
if (process.argv.includes("--self-test")) {
  ...
  const rejectPreGate = (label, source) => {
    let rejected = false;
    try { validate(config, runtimeConfig, healthConfig, packageJson, source); } catch { rejected = true; }
    if (!rejected) fail(`negative pre-gate self-test not rejected: ${label}`);
    cases += 1;
  };

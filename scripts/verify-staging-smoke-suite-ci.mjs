...
export function validate(config, runtimeConfig, healthConfig, packageJson) {
  ...
  if (packageJson?.scripts?.test !== "node scripts/verify-staging-smoke-suite-ci.mjs --self-test && node scripts/run-ci-tests.mjs") fail("aggregate npm test must gate 25.12");
...
const result = validate(config, runtimeConfig, healthConfig, packageJson);
...
if (process.argv.includes("--self-test")) {
  ...
  const rejectConfig = (label, mutate) => {
    ...
    mutate(candidate);
    let rejected = false;
    try { validate(candidate, runtimeConfig, healthConfig, packageJson); } catch { rejected = true; }
